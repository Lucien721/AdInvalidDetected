...
public class Translator {
     ...
     */
    protected BigInteger encode(String date, HighLevelDataType dataType) {
     ...
     */
    protected BigInteger encode(int l_H, String value) {
        BigInteger encodedValue = Utils.encode(l_H, value);
        addToTranslationMap(value, HighLevelDataType.STRING, encodedValue);
        return encodedValue;
    }

    protected String decode(BigInteger encodedValue) {
        String result;
        Object[] highLevelData = translationMap.get(encodedValue);
        if (highLevelData[1] == HighLevelDataType.STRING) {
            result = (String) highLevelData[0];
        } else {
            Date date = null;
            try {
                date = Utils.parseDate((HighLevelDataType) highLevelData[1],
                        (String) highLevelData[0]);
            } catch (ParseException e) {
                e.printStackTrace();
            }
            result = date.toString();
        }
        return result + ", " + highLevelData[1].toString();
    }
}
