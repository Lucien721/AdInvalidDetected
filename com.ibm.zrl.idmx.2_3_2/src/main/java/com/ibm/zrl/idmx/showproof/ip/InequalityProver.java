...
public class InequalityProver {
     ...
     * @return random value.
     */
    private static BigInteger genTildeRandom(final SystemParameters sp, int nPhi) {
        int bitlength = sp.getL_m() + sp.getL_H() + nPhi * sp.getL_Phi();
        return Utils.computeRandomNumber(bitlength);
    }
    ...
    public final Vector<BigInteger> computeTHatValues() {
        ...
        for (int i = 0; i <= NUM_SQUARES; i++) {
            rTildeValues[i] = genTildeRandom(sp, 2);
        }
        ...
        // compute THat_i
        for (int i = 0; i < NUM_SQUARES; i++) {
            uTildeValues[i] = genTildeRandom(sp, 1);
    ...
    public final HashMap<String, SValue> computeSValues(final BigInteger c) {
        ...
        for (int i = 0; i <= NUM_SQUARES; i++) {
            ...
            // this is also case for Delta
            rHat[i] = Utils.computeResponse(rTildeValues[i], c, rValues[i]);

            if (i == NUM_SQUARES) {
                // note how we preserve mHat for use in the verifier here.
                // rather than building up some arcane data structure, we simply
                // copy the value from the attribute (in fact, it's only a
                // pointer copying).
                // but for consistency we alias the attribute's mHat.

                // We retrieve the randomness of m (first argument) and m_r
                // (second argument) through dedicated methods as we want to
                // keep it transparent whether the second argument is a constant
                // or an attribute.
                // [spec: InequalityProver 3.1]
                uHat[NUM_SQUARES] = Utils.computeResponse(
                        pred.getFirstArgumentRandom(), c,
                        pred.getFirstArgumentValue());
                assert (uHat[NUM_SQUARES] != null);
            } else {
                // [spec: InequalityProver 3.2] compute deltaHat
