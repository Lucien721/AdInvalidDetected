...
public class Issuer {
    ...
    /**
     * Create an Issuer, to issuer a credential to a Recipient.
     * 
     * @param issuerKey
    ...
    public final Message round2(final Message msg) {
        ...
        if (nym != null) {
            nymHat = Utils.computeCommitment(gp, mHat_1,
                    msg.getProof().getCommonValue(IssuanceSpec.rHat)).modPow(
                    negC, capGamma);
        }
        if (domNym != null) {
            domNymHat = Utils
        ...
        BigInteger nonce_recipient = msg
                .getIssuanceElement(IssuanceProtocolValues.nonce_recipient);
        ...
        issuanceProtocolValues.put(IssuanceProtocolValues.vPrimePrime,
                vPrimePrime);
        issuanceProtocolValues.put(IssuanceProtocolValues.capQ, capQ);
        ...
        if (updateSpecLocation != null) {
            ...
            updateLocation = getIndividualUpdateLocation();
     ...
     */
    private URI getIndividualUpdateLocation() {
        return Locations.getUpdateLocation().resolve(
                "Update_" + Utils.getRandomString(20) + ".xml");
    }
     ...
     * @return capQ
     */
    public BigInteger computeQ(final BigInteger capS, final BigInteger capU,
            final BigInteger capZ, final BigInteger[] capR,
            final BigInteger vPrimePrime, final BigInteger n) {
        ...
        q = Utils.multiExpMul(capU, e, n);
        q = q.modInverse(n);
        q = (capZ.multiply(q)).mod(n);
    ...
    public static Message updateCredential(final IssuerKeyPair issuerKeyPair,
        ...
        for (AttributeStructure attStruct : attStructs) {
            ...
            final BigInteger mBar_i = (BigInteger) values.get(name)
                    .getContent();
