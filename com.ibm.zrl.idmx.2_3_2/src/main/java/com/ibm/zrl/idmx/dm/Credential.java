...
public class Credential {
    ...
    /**
     * (optional) Name of the credential - could be used to make them locally
     * identifiable.
     */
    private String name;

    // /** Bookkeeping of non-revealed attributes. */
    // private Vector<Attribute> nonRevealedAttrs = null;
     ...
     */
    public Credential(final URI theCredStructLocation,
        ...
        CredentialStructure credStruct = (CredentialStructure) StructureStore
                .getInstance().get(theCredStructLocation);

        ipk = credStruct.getPublicKey();
     ...
     *            Master secret object.
     */
    private void verifySignature(final MasterSecret masterSecret) {
    ...
    public final void update(final Message msg, final Values newValues) {
        ...
        final BigInteger capQ = msg
                .getIssuanceElement(IssuanceProtocolValues.capQ);
        ...
        if (!cHat.equals(msg.getProof().getChallenge())) {
            throw new RuntimeException(
                    "Verification failure! Mismatching cPrime, cHat");
        }
     ...
     */
    public final Vector<Attribute> getAttributes() {
        return attributes;
    }
